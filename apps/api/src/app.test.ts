...
const createPilotAuthSession = async (phone: string = TEST_USER_PHONE): Promise<PilotAuthSession> => {
    await requestPilotOtp(phone);

    const verify = await verifyPilotOtp(phone);
...
describe("NaijaRides API endpoints", () => {
    ...
    it("GET /rides/:rideId/passengers", async () => {
        const driverSession = await createPilotAuthSession(TEST_USER_PHONE);
        const riderSession = await createPilotAuthSession("+2348000000001");

        const create = await withPilotAuth(request(app).post("/rides"), driverSession)
            .send({
                from: "Yaba",
                to: "Victoria Island",
                time: "9:00 AM",
        ...
        const join = await withPilotAuth(request(app).post(`/rides/${create.body.id}/join`), riderSession)
            .send({});
        expect([200, 204]).toContain(join.status);

        const asDriver = await withPilotAuth(request(app).get(`/rides/${create.body.id}/passengers`), driverSession);
        expect(asDriver.status).toBe(200);
        expect(Array.isArray(asDriver.body)).toBe(true);
        expect(asDriver.body.length).toBe(1);
        expect(asDriver.body[0]).toMatchObject({
        ...
        const asRider = await withPilotAuth(request(app).get(`/rides/${create.body.id}/passengers`), riderSession);
        expect(asRider.status).toBe(403);
        expect(asRider.body).toEqual({ error: "Only the driver can view passengers for this ride" });
    });
